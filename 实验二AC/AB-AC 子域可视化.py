...
# ⚠️ 强制使用 CPU，保证物理计算精度 (Float64)
device = torch.device("cpu")
dtype = torch.float64
torch.set_default_dtype(dtype)

print(f"🔥 任务: AB-PINN Hard-Constraint AC (L-BFGS + Visualization)")
print(f"⚠️  注意: 已强制使用 CPU (Float64)")
...
class PeriodicEmbedding(nn.Module):
    """ Fourier embedding: x -> [cos(pi*x), sin(pi*x)] """
    def __init__(self):
        super().__init__()
    def forward(self, x):
...
class ABPINN_Hard(nn.Module):
    ...
    def add_expert(self, center_vec):
        ...
        # [修改] 初始半径从 0.5 减小到 0.3，使其更"尖锐"，专注修补局部细节
        expert = LocalExpert(self.input_dim, hidden=20, center=center_vec, radius=0.3).to(device)
        self.experts.append(expert)
        print(f"   🌱 Added Expert at: {center_vec.detach().cpu().numpy().flatten().round(2)}")
...
# 4. 训练循环 (Adam + L-BFGS)
# ==========================================================
MAX_EXPERTS = 6          # 允许更多一点专家
ITER_PER_ADD = 4000      # 稍微加快添加频率
ADAM_ITERS = 30000       # Adam 总步数
FREEZE_ITER = 25000      # 冻结窗口时间
...
for it in range(ADAM_ITERS + 1):
    ...
    if it > 0 and it % ITER_PER_ADD == 0 and len(model.experts) < MAX_EXPERTS:
        x_scan = torch.linspace(-1, 1, 120, device=device, dtype=dtype).view(-1,1)
        t_scan = torch.linspace(0, 1, 60, device=device, dtype=dtype).view(-1,1)
        ...
        max_val, idx = torch.max(res_abs, 0)
        best_x = x_flat[idx]
        best_t = t_flat[idx]
        
        print(f"🔍 Iter {it}: Max Residual {max_val.item():.4f} at (x={best_x.item():.2f}, t={best_t.item():.2f})")
...
# 5. L-BFGS 微调 (新增阶段)
# ==========================================================
print("\n>>> 🚀 Phase 2: L-BFGS Fine-tuning (Fixing the curve)...")
# 使用大Batch进行全场精修
...
# 3. [新增] 子域分布图 (Subdomain Distribution)
axs[2].set_title("Subdomain Distribution (Window Contours)")
axs[2].set_xlabel("t"); axs[2].set_ylabel("x")
axs[2].set_xlim(0, 1); axs[2].set_ylim(-1, 1)

# 背景放残差 (可选，这里放预测解的轮廓淡一点)
axs[2].contourf(T_mesh, X_mesh, u_pred, 20, cmap='gray', alpha=0.2)
...
with torch.no_grad():
    for i, expert in enumerate(model.experts):
        ...
        # 2. 画轮廓 (Level set = 0.5 表示窗口的有效范围)
        # 使用不同的颜色区分
        color = plt.cm.tab10(i % 10)
        axs[2].contour(T_mesh, X_mesh, phi_map, levels=[0.5], colors=[color], linewidths=2)
        
        # 3. 画中心点 (反解回 x,t 坐标比较麻烦，这里直接找 phi 的最大值位置)
        # 因为 mu 是在嵌入空间定义的，直接画出来不直观，我们用 mesh 上 phi 最大的点代替中心
        max_idx = np.argmax(phi_map)
        center_t = T_mesh.flatten()[max_idx]
        center_x = X_mesh.flatten()[max_idx]
        axs[2].scatter(center_t, center_x, marker='x', s=100, color=color, label=f'Exp {i+1}')

axs[2].legend(loc='upper right', fontsize='small', framealpha=0.8)
...
plt.tight_layout()
plt.savefig("ABPINN_LBFGS_Vis.png", dpi=150)
print("✅ Done.")
plt.show()