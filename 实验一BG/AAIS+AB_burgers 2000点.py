...
class DynamicABPINN(nn.Module):
    ...
    def add_subdomain(self, mu_init):
        ...
        lg = nn.Parameter(torch.tensor([2.5], device=device, dtype=dtype)) # 稍微降低初始锐度，防止震荡
...
def redistribute_points_hybrid(model, n_total, current_nu):
    """ 混合采样：40% 均匀 (增加均匀比例保底) + 60% 自适应 """
    model.eval()
    
    n_uniform = int(0.4 * n_total) # 提高到底色比例
...
t_bc = torch.rand(400, 1, device=device, dtype=dtype) # 增加边界点数
...
def compute_loss_val(model, x_r, t_r, nu_val):
    ...
    return loss_r + 100.0 * (loss_ic + loss_bc), res # 权重平衡一下
...
for epoch in range(ADAM_EPOCHS + 1):
    ...
    if epoch > 2000 and epoch % 3000 == 0:
        ...
        if len(model.subnets) < 6 and max_res > 0.02: # 阈值稍微降低
            idx = torch.argmax(res_tensor.abs())
            new_mu = [x_r[idx].item(), t_r[idx].item()]
            model.add_subdomain(new_mu)
            print(f"⚡️ [Add Subnet] -> Total: {len(model.subnets)}")
            optimizer = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-6)
...
# 简单绘图
model.eval()
u_pred = model(X_star, T_star).detach().cpu().numpy().reshape(len(t_exact), len(x_exact)).T

plt.figure(figsize=(10, 5))
idx = int(0.5 * len(t_exact))
plt.plot(x_exact, u_exact_all[:, idx], 'k-', linewidth=3, label="Exact")
plt.plot(x_exact, u_pred[:, idx], 'r--', linewidth=2, label="Pred")
plt.title(f"Check at t=0.5 (Err={final_error:.4f})")
plt.legend()
plt.savefig("corrected_result.png")