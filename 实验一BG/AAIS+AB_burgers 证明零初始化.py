...
# ==========================================================
# 0. 基础配置 (CPU + Float64) - 保持与 Code 5 一致
# ==========================================================
...
print(f"🔥 设备: {device} | 精度: Float64 (Model B: No Zero Init Control)")
...
class DeepSubNet(nn.Module):
    def __init__(self, hidden=64, is_new_addition=False):
        ...
        # 🔥【关键对照点】即便传入 is_new_addition=True，
        # 如果我们在调用时传入 False，这段逻辑就不会执行。
        # 本代码将在 DynamicABPINN 中强制传入 False。
        if is_new_addition:
...
class DynamicABPINN(nn.Module):
    ...
    def add_subdomain(self, mu_init):
        # 🔥【修改点】强制设为 False！
        # 这意味着新网络将带有随机的 Xavier 初始化权重，
        # 而不是 0 输出。这会瞬间引入误差。
        new_net = DeepSubNet(self.hidden, is_new_addition=False).to(device)
...
# Phase 1: Adam
# ==========================================================
print(">>> Phase 1: Adam (With Random Init Subdomains)...")
...
for epoch in range(ADAM_EPOCHS + 1):
    ...
    if epoch > 2000 and epoch % 3000 == 0:
        ...
        if len(model.subnets) < 6 and max_res > 0.05:
            ...
            print(f"⚠️ 自适应触发 (Random Init): Max Res={max_res:.4f} -> Add SubNet {len(model.subnets)+1}")
            print(f"   [Observation]: Watch for Loss Spike!")
            
            # 这里调用的是修改后的 add_subdomain (is_new_addition=False)
...
print(f"\n✨ Final Relative L2 Error (No Zero Init): {error_l2:.4e}")

# 绘图 - 重点展示 Loss 曲线
plt.figure(figsize=(14, 6))

plt.subplot(1, 2, 1)
plt.semilogy(loss_history, label='Loss (Random Init)')
plt.title("Loss History: Observe Spikes at Addition")
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.legend()
...
# 标记添加子网的时刻 (大概位置)
for x_line in range(3000, 15000, 3000):
    plt.axvline(x=x_line, color='r', linestyle='--', alpha=0.5)

plt.subplot(1, 2, 2)
idx = int(0.5 * len(t_exact))
plt.plot(x_exact, u_exact_all[:, idx], 'k-', linewidth=3, label="Exact")
plt.plot(x_exact, u_pred[:, idx], 'r--', linewidth=2, label="Pred (No Zero Init)")
plt.title(f"Comparison t=0.5 (Err: {error_l2:.2e})")
plt.legend()
plt.tight_layout()

plt.savefig('model_b_no_zero_init.png', dpi=300)
print("✅ 结果已保存为 model_b_no_zero_init.png")