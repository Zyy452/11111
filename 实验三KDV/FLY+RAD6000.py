...
def generate_aais_points(model, n_points=6000): # ⚠️ 压力测试：点数砍到 6000
...
# ================= 4. Phase 1: Adam =================
print("🔥 开始 Phase 1: 压力测试 (6000点，无专家网络)")
time_start_global = time.time() # ⚠️ 新增：记录全局起始时间
time_start_phase1 = time.time() # ⚠️ 新增：记录 Phase 1 起始时间

optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
x_f, t_f = generate_aais_points(model, n_points=6000) # ⚠️ 压力测试初始点数

for it in range(15001):
    if it > 2000 and it % 2000 == 0 and it < 14000:
        x_f, t_f = generate_aais_points(model, n_points=6000) # ⚠️ 压力测试重采样
...
print(f"⏱️ Phase 1 耗时: {time_end_phase1 - time_start_phase1:.2f} 秒") # ⚠️ 新增：打印 Phase 1 耗时

# ================= 5. Phase 2: L-BFGS =================
print("\n🚀 开始 Phase 2: L-BFGS...")
time_start_phase2 = time.time() # ⚠️ 新增：记录 Phase 2 起始时间

x_anchor = torch.rand(2000, 1, device=device, dtype=dtype) * (x_max - x_min) + x_min # ⚠️ 锚点也同步减少
t_anchor = torch.rand(2000, 1, device=device, dtype=dtype) * (t_max - t_min) + t_min
...
for stage in range(3):
    ...
    x_rad, t_rad = generate_aais_points(model, n_points=6000) # ⚠️ 压力测试重采样
...
print(f"⏱️ Phase 2 耗时: {time_end_phase2 - time_start_phase2:.2f} 秒") # ⚠️ 新增：打印 Phase 2 耗时
...
with torch.no_grad():
    ...
    max_error = np.max(np.abs(u_exact - u_pred)) # ⚠️ 核心指标！
    
    print(f"✅ =============================================")
    print(f"✅ [稀疏测试] 消融 B 最终相对 L2 误差 (平均): {final_error:.6f}")
    print(f"🔥 [稀疏测试] 消融 B 最终最大绝对误差 (最差): {max_error:.6f}")
    print(f"⏱️  总耗时: {total_mins} 分 {total_secs} 秒 ({total_time:.2f} 秒)") # ⚠️ 新增：打印总耗时
    print(f"✅ =============================================")
...
ax2.set_title("Absolute Error (Sparse 6000 points)")
...
plt.tight_layout()
plt.savefig(os.path.join(SAVE_DIR, "Fourier_RAD_6000_KdV.png"), dpi=300)
