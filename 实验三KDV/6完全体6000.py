...
def generate_aais_points(model, n_points=6000): # ⚠️ 修改：稀疏采样 6000
...
# ================= 4. Phase 1: Adam 预训练 =================
print("🔥 开始 Phase 1: 压力测试完全体 (6000点，有专家网络)")
time_start_global = time.time() # ⚠️ 新增：记录全局起始时间
time_start_phase1 = time.time() # ⚠️ 新增：记录 Phase 1 起始时间
optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)

x_f, t_f = generate_aais_points(model, n_points=6000) # ⚠️ 修改：稀疏采样 6000
...
x_anchor = torch.rand(6000, 1, device=device, dtype=dtype) * (x_max - x_min) + x_min # ⚠️ 修改：锚点降至 2000
...
for stage in range(3):
    ...
    x_rad, t_rad = generate_aais_points(model, n_points=6000) # ⚠️ 修改：稀疏采样 2000
...
plt.tight_layout()
plt.savefig(os.path.join(SAVE_DIR, "RAD_AB_Fourier_6000_KdV1.png"), dpi=300)
