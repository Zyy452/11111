...
def generate_aais_points(model, n_points=N_TRAIN_POINTS): 
...
# ================= 4. Phase 1: Adam 预训练 =================
print(f"\n🔥 开始 Phase 1: 压力测试完全体 ({N_TRAIN_POINTS}点，有专家网络)")
time_start_global = time.time() 
time_start_phase1 = time.time() 

optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
x_f, t_f = generate_aais_points(model, n_points=N_TRAIN_POINTS) 
...
x_anchor = torch.rand(N_TRAIN_POINTS, 1, device=device, dtype=dtype) * (x_max - x_min) + x_min 
t_anchor = torch.rand(N_TRAIN_POINTS, 1, device=device, dtype=dtype) * (t_max - t_min) + t_min
...
for stage in range(3):
    ...
    x_rad, t_rad = generate_aais_points(model, n_points=N_TRAIN_POINTS) 
...
save_data = {
    ...
    "n_points": N_TRAIN_POINTS # 顺便把点数也存入数据包
}

# 🌟 动态生成保存文件名，带上当前的点数参数
DATA_SAVE_PATH = os.path.join(SAVE_DIR, f"KDV_fourier_pts{N_TRAIN_POINTS}_results.pt") 
torch.save(save_data, DATA_SAVE_PATH)

print(f"\n✅ =============================================")
print(f"✅ [{N_TRAIN_POINTS}点测试] 完全体 最终相对 L2 误差 (平均): {final_error:.6f}")
print(f"🔥 [{N_TRAIN_POINTS}点测试] 完全体 最终最大绝对误差 (最差): {max_error:.6f}")
